...
export class CreateUserDto {
    ...
    @ApiProperty({
        example: 'Senha@123!',
        description:
            'Senha forte (mín. 8 chars, maiúscula, minúscula, número e símbolo)',
    })
    @IsStrongPassword({
        message:
    ...
    @ApiPropertyOptional({
        example: 2,
        description: 'ID numérico do papel (role). Omita para usar o padrão.',
    })
    @IsOptional()
    ...
    @ApiPropertyOptional({
        description:
            'ID do usuário criador (preenchido automaticamente quando autenticado)',
    })
    @IsOptional()
    ...
    @ApiProperty({
        ...
        description: 'UUID do serviço ao qual o usuário pertence',
    })
    @IsUUID()
    serviceId: string;
}
